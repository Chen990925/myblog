...
**那么间隙就有 id 为 (3,10)，(10,20)，(20,正无穷) 这三个区间**，在事务_1下面执行 `update account set name = 'cjl' where id > 8 and id <18;`，则其他事务没法在这个**范围所包含的所有行记录以及行记录所在的间隙**里插入或修改任何数据，即id在(3,20]区间都无法修改数据，注意最后那个20也是包含在内的。
