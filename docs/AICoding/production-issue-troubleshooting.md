...
- **排查**：`jstack` 几百线程全 WAITING、栈底 Feign 同步调用；结合监控 CFETS P99 200ms→30s+；查 Feign 配置发现 **`readTimeout=600000`（600s = 10 分钟）** 且无熔断
- **根因**：**Feign readTimeout 过长（600s）** + **无熔断降级（项目未引入 Sentinel/Hystrix，pom 无依赖）** + 关键链路同步调用未隔离；网关连接被占满，雪崩向上游蔓延
- **⚠️ 项目真实状态 + 改进方案**（不要讲成"已接入 Sentinel"！）：
  ```
  【已落地】回报链路改 RocketMQ 异步化（不依赖外部同步返回）
           健康检查独立路径，保证监控能拉起
  【改进方案·未落地】超时分级收紧（连接 3s / 读 10s）——当前是 10s/600s
           引入 Sentinel 熔断降级（失败率超阈值快速失败）
           CFETS 调用信号量隔离，防止占满整个线程池
  → 目标：故障从全链路瘫痪收敛为单点快速失败
  ```
